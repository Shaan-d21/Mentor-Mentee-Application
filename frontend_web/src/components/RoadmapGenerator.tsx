...
const RoadmapGenerator: React.FC = () => {
    ...
    const fetchMentees = async () => {
        try {
            ...
            console.log('Making request to:', '/api/mentor/get-approved-mentee');

            const response = await axios.get('/api/mentor/get-approved-mentee', {
                headers: {
                    'Token': token,
            ...
            if (response.data && response.data.object) {
                ...
                if (Array.isArray(menteeList) && menteeList.length > 0) {
                    // Check for existing roadmaps for each mentee
                    const menteesWithRoadmapStatus = await Promise.all(
                        menteeList.map(async (mentee) => {
                            try {
                                const roadmapResponse = await axios.get(`/api/mentor/check-roadmap/${mentee.id}`, {
                                    headers: {
                                        'Token': token,
                                        'Content-Type': 'application/json',
                                        'Accept': 'application/json'
                                ...
                                return {
                                    ...mentee,
                                    has_roadmap: roadmapResponse.data?.has_roadmap || false
                                };
                            } catch (error) {
                                console.error(`Error checking roadmap for mentee ${mentee.id}:`, error);
                                return {
                                    ...mentee,
                                    has_roadmap: false
                                };
    ...
    const handleGenerateRoadmap = async () => {
        ...
        try {
            ...
            const response = await axios.post(
                ...
                {
                    title: `${selectedMenteeData.name}'s Roadmap`,
            ...
            if (response.data) {
                ...
                // Update topics with the received roadmap data
