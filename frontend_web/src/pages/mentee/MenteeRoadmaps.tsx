...
const MenteeRoadmaps: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchMentors = async () => {
      try {
        ...
        console.log('Making API request to /api/mentee/mentor-roadmap-details');
        console.log('Using token:', authToken.substring(0, 20) + '...');
        
        const response = await axios.get('/api/mentee/mentor-roadmap-details', {
          headers: {
            'Token': authToken,
        ...
        if (response.data) {
          if (response.data.object) {
            console.log('Response object:', response.data.object);
    ...
    if (location.pathname.includes('/roadmaps')) {
      console.log('Fetching mentors for roadmaps page');
      fetchMentors();
    }
  }, [location.pathname]);
  ...
  const handleViewRoadmap = async (mentor: Mentor) => {
    try {
      ...
      const response = await axios.post('/api/mentee/roadmap-topics', {
        mentor_id: mentor.mentor_id,
        domain_id: mentor.domain_id
      }, {
        headers: {
