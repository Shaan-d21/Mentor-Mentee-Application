...
const CheckCompatibility:FC<ScreenProps<"CheckCompatibility">> = ({navigation}) => {
  const dispatch = useDispatch<AppDispatch>();
  const [value, setValue] = useState('');
  const [modalVisible, setModalVisible] = useState(false);
  const [selectedReason, setSelectedReason] = useState('');
  ...
  // ...existing code...
const submitDomain = async () => { // Make submitDomain async
  ...
  return (
    <View style={styles.container}>
      <AppBar onProfilePress={() => { navigation.navigate('MenteeProfileScreen')}
    } openDrawer={() => { }} />
  ...
  {loading ? (
    ...
    <Text style={styles.findMentorsButtonText}>Find Mentors</Text> // Show text
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {domain_mentors!=null && <Text style={styles.sectionTitle}>Domain Mentors</Text>}
        ...
        {domain_mentors?.length === 0 ? (
          ...
          domain_mentors?.map((mentor: Mentor) => (
            <View key={mentor.id} style={styles.menteeRequest}>
              <View style={styles.mentorInfo}>
                <Text style={styles.nameText} numberOfLines={1} ellipsizeMode="tail">{mentor.name}</Text>
                <View style={styles.infoRow}>
                  <FontAwesomeIcon icon={faEnvelope} size={16} style={styles.icon} />
                  <Text style={styles.infoText} numberOfLines={1} ellipsizeMode="tail">{mentor.mail}</Text>
                </View>
                <View style={styles.infoRow}>
                  <FontAwesomeIcon icon={faBriefcase} size={16} style={styles.icon} />
                  <Text style={styles.infoText} numberOfLines={1} ellipsizeMode="tail">{mentor.designation}</Text>
                </View>
                <View style={styles.infoRow}>
                  <FontAwesomeIcon icon={faUserGraduate} size={16} style={styles.icon} />
                  <Text style={styles.domainText} numberOfLines={2} ellipsizeMode="tail">{mentor.domain}</Text>
                </View>
              </View>
        ...
        {other_domain_mentors!=null && <Text style={styles.sectionTitle}>Other Domain Mentors</Text>}
        {other_domain_mentors?.length === 0 ? (
          <Text style={styles.noRequestsText}>No other domain mentors found at this moment</Text>
        ) : (
          other_domain_mentors?.map((mentor: Mentor) => (
